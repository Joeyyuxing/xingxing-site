...
function addCmt(id){const nE=document.querySelector(`[data-cn="${id}"]`),tE=document.querySelector(`[data-ci="${id}"]`);if(!tE||!tE.value.trim())return;const nick=nE?.value.trim();if(!nick){toast('请先填写你的名字 ✏️');if(nE){nE.focus();nE.classList.add('shake');setTimeout(()=>nE.classList.remove('shake'),400)}return}myNick=nick;localStorage.setItem('xpz_nick',nick);const p=posts.find(x=>x.id===id);if(!p)return;const newComment={author:nick,text:tE.value.trim(),time:new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})};p.comments.push(newComment);cloudUpdatePost(id,{comments:p.comments});savePosts().then(()=>{render();setTimeout(()=>{const el=document.querySelector(`[data-cmt="${id}"]`);if(el)el.classList.add('open')},50)})}
